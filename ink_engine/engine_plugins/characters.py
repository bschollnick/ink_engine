...
def _bind(state_dict: dict[str, Any], engine_state: dict[str, Any]) -> dict[str, Callable[..., Any]]:
    ...
    Args:
        ...
        engine_state: The full session state, read directly (not via any
            dynamic ownership-resolution mechanism) for other plugins'
            state slots this plugin reads but does not own -- here,
            `character_occupancy`'s own slot, by its known literal key.
            Used only for the delegating accessor below, never written to.
    ...
    def current_location_now(character_id: str) -> str:
        ...
        return current_location_of(engine_state.get("character_occupancy", {}), character_id)
...
# The plugin-discovery contract (ink_engine/discovery.py). Reading
# character_occupancy's own state slot (for current_location_now) is a
# plain engine_state.get("character_occupancy", {}) inside _bind()
# itself -- the mechanism that lets a character be the one thing a story
# asks, without this module becoming a second home for another plugin's
# state, with no also_reads declaration needed.
PLUGIN = Plugin(
    name="characters",
