...
from ink_engine.engine_config_schemas import validate_cost_table
from ink_engine.plugin import Plugin

# The state slot this plugin owns, named as a module constant so a
# dependent plugin's own `_bind` can read it via
# `engine_state.get(STATE_KEY, {})` without a hardcoded string literal.
...
def _init_cost_table_state() -> dict[str, Any]:
    ...
    arguments, so it cannot see the story's config. A story builds its
    real starting table with `initial_state(config)` from its own
    initializer.
...
def _bind(state_dict: dict[str, Any], engine_state: dict[str, Any]) -> dict[str, Callable[..., Any]]:
    ...
    Args:
        ...
        engine_state: The full session state. Unused -- this plugin reads
            ...
            is always this exact two-argument shape, so every plugin
            accepts it even when it has nothing to read from it.
