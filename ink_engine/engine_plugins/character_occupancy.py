...
`location_graph.STATE_KEY` (imported directly, no dynamic ownership
resolution -- see the design doc's "also_reads was dead generality"
finding), not an import of `location_graph`'s own functions: the
vocabulary is read from the session's own location state, so a story
that declares no map is simply unchecked rather than broken.
...
from ink_engine.engine_config_schemas import validate_character_occupancy
...
def _bind(state_dict: dict[str, Any], engine_state: dict[str, Any]) -> dict[str, Callable[..., Any]]:
    ...
    Per `Plugin.bind`'s own contract: `state_dict` is this ONE session's
    own dict — never shared or reused across sessions, since a host's own
    binding-resolution call is itself made fresh per request.
    `set_location`/`where_is`/`who_is_at` (above) stay pure functions of
    an explicit `OccupancyState` in, value/new-state out; this wrapper is
    the only place that adapts them to read/mutate `state_dict` in place,
    so a write survives exactly as long as `state_dict` itself does (a
    session's own persisted engine state) with no change to their own
    tested signatures.
    ...
    Args:
        state_dict: This session's own `OccupancyState.to_dict()` result,
            ...
            `set_location_now`.
        engine_state: The full session state, read directly (not via any
            dynamic ownership-resolution mechanism) for the location
            map's own declared vocabulary, keyed by its known
            `location_graph.STATE_KEY` constant — which places exist. A
            write to a location the map does not declare is a typo, and
            raises rather than silently removing the character from the
            game. A story with no map slot is not checked.
    ...
    known_locations = frozenset(engine_state.get(_LOCATION_STATE_KEY, {}).get("declared", ()))
...
# The plugin-discovery contract (ink_engine/discovery.py) — set_location()/
# where_is()/who_is_at() are exposed as real EXTERNAL bindings via
# state_key/init_state/bind, closures adapted per-session by _bind()
# above (see its own docstring for why this needs a wrapper rather than
# binding the pure functions directly). Reading location_graph's own
# state slot (for the declared-locations check) is a plain
# engine_state.get(_LOCATION_STATE_KEY, {}) inside _bind() itself -- no
# also_reads declaration needed, since the owning plugin's state_key is
# already known statically via the imported constant.
PLUGIN = Plugin(
