...
Config shape: this plugin's own config JSON (system_name
`"location_graph"`, discovered via this module's own `PLUGIN` below),
validated by `engine_config_schemas.validate_location_graph()` —
`{"locations": {location_id: {"known_by_default": bool,
...
# The plugin-discovery contract (ink_engine/discovery.py) — this module
# exposes no EXTERNAL-dispatched bindings of its own (its functions
# operate on a story's own explicitly-threaded state, not on Ink's
# eval_stack directly), only a config schema and a state slot. A
# dependent plugin (character_occupancy.py) reads this slot's declared
# vocabulary directly via its own known STATE_KEY constant -- no dynamic
# ownership resolution needed (see the design doc's "also_reads was dead
# generality" finding).
...
def _init_location_graph_state() -> dict[str, Any]:
    ...
    Empty rather than config-derived: `Plugin.init_state` takes no
    arguments, so it cannot see the story's own config. A story wanting
    its map's real starting state calls `initial_state(config)` itself,
    as the reference game does from its own `init_state`.
...
def _bind(own_state: dict[str, Any], engine_state: dict[str, Any]) -> dict[str, Any]:
    """This plugin's own EXTERNAL bindings -- none. Kept as an explicit,
    documented no-op `bind` (rather than omitting `bind`/`state_key`/
    `init_state` entirely) because this plugin genuinely does own real
    per-session state (the discovered/declared/visited location sets),
    even though nothing in that state is exposed as a direct Ink
    EXTERNAL call -- every real read/write goes through the `*_in`
    functions above, called by a DEPENDENT plugin's own `bind`.
    """
    return {}
