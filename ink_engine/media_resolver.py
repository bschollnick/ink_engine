...
**Why the standalone resolver does not (and must not) resemble QuickBBS's
own DB-indexed lookup**: QuickBBS's `StoryImage`/`FileIndex` indirection
exists specifically so a game folder does not have to duplicate a directory
tree that already exists somewhere in the scanned Albums gallery — the tag
is deliberately NOT the path there. A standalone game folder carries none
of that constraint: everything the game needs already ships inside its own
folder, so the tag IS the path, directly and only. `FilesystemMediaResolver`
below does a plain per-tag file-existence check under the game's own
directory — no flat `media/` catch-all, no fixed per-kind subdirectory, no
filename-stem matching, no override dictionary, no upfront directory scan.
The game controls where its own files live; this resolver never polices
that — it only ever reads the path the tag itself already names. A
QuickBBS-side resolver answering the SAME tag vocabulary from its own
DB-indexed store is a wholly separate implementation of the same
`MediaResolver` seam; the two must never be confused with each other, and
neither one's behavior leaks into the other's module.

claude_docs/plans/standalone_if_player.md Step 2/3 (QuickBBS repository) —
this module implements the shared half described there.
"""
...
class FilesystemMediaResolver:  # pylint: disable=too-few-public-methods
    ...
    folder needs none of the indirection QuickBBS's own DB-indexed
    resolver exists for.
    """
    ...
    def resolve(self, requests: list[tuple[str, str]]) -> list[str]:
        ...
        Returns:
            ...
            into a `file://` URI or a served-loopback path is the player
            shell's own concern (Step 4), not this resolver's.
        """
