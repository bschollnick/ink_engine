"""sequences/cycles/shuffles + seeded RNG (ink_engine.engine).
...
class NetRandomTests(SimpleTestCase):
    ...
    `new System.Random(seed).Next()` across 13 seeds spanning ordinary
    values, ±1, and the int32 boundary (Int32.MaxValue, Int32.MinValue,
    and values one away from each)."""
...
class SequenceTests(SimpleTestCase):
    """`{a|b|c}` sequences, capped at the last element once
    exhausted (seq2.ink, seq_sticky.ink)."""

    def test_once_only_choices_prune_after_two_visits(self):
        """A `*`-choice loop only offers "Again" once; the sequence
        advances a-then-b across the two visits (seq2.ink)."""
        state = _run("seq2.ink.json")
    ...
    def test_sequence_caps_at_last_element_once_exhausted(self):
        ...
        wrapping (seq_sticky.ink)."""
        state = _run("seq_sticky.ink.json")
...
class CycleTests(SimpleTestCase):
    """`{&a|b|c}` cycles, wrapping via modulo instead of
    capping (cycle.ink, cycle_sticky.ink)."""

    def test_once_only_choices_prune_after_two_visits(self):
        """Matches SequenceTests' shape but with a cycle operator
        (cycle.ink)."""
        state = _run("cycle.ink.json")
        text = state.continue_story()
        self.assertEqual(text, "a\n")
        state.choose(0)
        text = state.continue_story()
    ...
    def test_cycle_wraps_around_via_modulo(self):
        ...
        exactly (cycle_sticky.ink)."""
        state = _run("cycle_sticky.ink.json")
        results = [state.continue_story()]
        for _ in range(4):
            state.choose(0)
            results.append(state.continue_story())
...
class ShuffleTests(SimpleTestCase):
    """`{~a|b|c}` shuffles, deterministic once SEED_RANDOM is
    called explicitly (shuffle_seeded.ink)."""
    ...
    def test_seeded_shuffle_matches_real_transcript(self):
        ...
        state = _run("shuffle_seeded.ink.json")
        text = state.continue_story()
        self.assertEqual(text, "a\n")
        state.choose(0)
        text = state.continue_story()
...
class RandomTests(SimpleTestCase):
    """SEED_RANDOM()/RANDOM(min, max) (random.ink)."""
...
class ListRandomTests(SimpleTestCase):
    ...
    def test_seeded_list_random_matches_real_transcript(self):
        ...
        state = _run("lrnd.ink.json", with_list_defs=True)
        text = state.continue_story()
    ...
    def test_list_random_of_empty_list_degrades_to_empty_list(self):
        ...
        than crashing (no real transcript needed — a standing defensive-
        degradation contract, unit-tested directly)."""
        state = InkRuntimeState(load_story_root(_load("lrnd.ink.json")))
